...
/* ─────────── /api/cerydra/balance ─────────── */
app.get("/api/cerydra/balance", async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT id, name, costs
      FROM cerydra_costs
...
/* ─────────── /api/admin/cerydra-balance ─────────── */
app.put("/api/admin/cerydra-balance", requireAdmin, async (req, res): Promise<void> => {
  const { characters } = req.body;
...
app.get("/api/cerydra/cone-balance", async (req, res) => {
  try {
    const { rows } = await pool.query(`
      SELECT id, name, costs, image_url as "imageUrl", subname, rarity
...
/* ─────────── /api/admin/cerydra-cone-balance ─────────── */
app.put("/api/admin/cerydra-cone-balance", requireAdmin, async (req, res): Promise<void> => {
